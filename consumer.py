...
def get_request(s3, bucket_name):
    ...
    next_key = keys[0]
    obj = s3_client.get_object(Bucket=bucket_name, Key=next_key)
...
def main():
    ...
    while True:
        request = get_request(s3, "bucket2")
        if request:
            process_request(request, args.storage, s3, dynamo, args.target)
        else:
